...
def moment_y_reference_markdown(
    ...
    moment_y_ref = moment_y_reference(
        moment_y_p, force_x, force_z, x_coord_p, z_coord_p
    )
    # for LaTeX
    # pylint: disable=anomalous-backslash-in-string
    markdown_formula = """$$
    M_{yU} = M_{yP} + F_{x}\cdot z_{P} - F_{z}\cdot x_{P}
    $$
    """

    markdown_filled = (f"$${moment_y_ref} = "
    f"{moment_y_p} + {force_x} \cdot {z_coord_p} - {force_z} \cdot {x_coord_p}$$")
